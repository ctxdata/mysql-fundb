...
/**
  ...
  /storage/fun/ha_fun.h.

  @details
  ha_fun will let you create/open/delete tables, but
  nothing further (for fun, indexes are not supported nor can data
  be stored in the table). Use this fun as a template for
  implementing the same functionality in your own storage engine. You
  can enable the fun storage engine in your build by doing the
  following during your build process:<br> ./configure
  --with-fun-storage-engine

  Once this is done, MySQL will let you create tables with:<br>
  CREATE TABLE \<table name\> (...) ENGINE=FUN;

  The fun storage engine is set up to use table locks. It
  implements an fun "SHARE" that is inserted into a hash by table
  name. You can use this to store information of state that any
  fun handler object will be able to see when it is using that
  table.

  Please read the object definition in ha_fun.h before reading the rest
  of this file.

  @note
  When you create an FUN table, the MySQL Server creates a table .frm
  (format) file in the database directory, using the table name as the file
  name as is customary with MySQL. No other files are created. To get an idea
  of what occurs, here is an fun select that would do a scan of an entire
  table:

  @code
  ha_fun::store_lock
  ha_fun::external_lock
  ha_fun::info
  ha_fun::rnd_init
  ha_fun::extra
  ha_fun::rnd_next
  ...
  Here you see that the fun storage engine has 9 rows called before
  rnd_next signals that it has reached the end of its data. Also note that
  the table in question was already opened; had it not been open, a call to
  ha_fun::open() would also have been necessary. Calls to
  ha_fun::extra() are hints as to what will be occurring to the request.

  A Longer Fun can be found called the "Skeleton Engine" which can be
...
/**
  @brief
  Fun of simple lock controls. The "share" it creates is a
  structure we will pass to each fun handler. Do you have to have
...
static handler *fun_create_handler(handlerton *hton, TABLE_SHARE *table,
  ...
  return new (mem_root) ha_fun(hton, table);
}
...
*/
static st_handler_tablename ha_fun_system_tables[] = {
...
/**
  ...
  See ha_tina.cc for an fun of extracting all of the data as strings.
  ha_berekly.cc has an fun of how to store it intact by "packing" it
...
*/
int ha_fun::update_row(const uchar *, uchar *) {
  DBUG_TRACE;
...
int ha_fun::index_read_map(uchar *, const uchar *, key_part_map,
...
/**
  ...
  scan. See the fun in the introduction at the top of this file to see when
  rnd_init() is called.
...
*/
void ha_fun::position(const uchar *) { DBUG_TRACE; }
...
*/
int ha_fun::rnd_pos(uchar *, uchar *) {
  int rc;
  DBUG_TRACE;
  rc = HA_ERR_WRONG_COMMAND;
  return rc;
...
/**
  ...
  ha_innodb.cc
*/
int ha_fun::extra(enum ha_extra_function) {
  DBUG_TRACE;
  return 0;
}
...
*/
int ha_fun::external_lock(THD *, int) {
  DBUG_TRACE;
  return 0;
}
...
/**
  ...
  Berkeley DB, for fun, changes all WRITE locks to TL_WRITE_ALLOW_WRITE
...
*/
THR_LOCK_DATA **ha_fun::store_lock(THD *, THR_LOCK_DATA **to,
...
*/
int ha_fun::rename_table(const char *, const char *, const dd::Table *,
                             dd::Table *) {
  DBUG_TRACE;
  return HA_ERR_WRONG_COMMAND;
}
...
*/
ha_rows ha_fun::records_in_range(uint, key_range *, key_range *) {
  DBUG_TRACE;
...
static SYS_VAR *fun_system_variables[] = {
...
// this is an fun of SHOW_FUNC
static int show_func_fun(MYSQL_THD, SHOW_VAR *var, char *buf) {
  var->type = SHOW_CHAR;
...
fun_vars_t fun_vars = {100, 20.01, "three hundred", true, false, 8250};

static SHOW_VAR show_status_fun[] = {
    {"var1", (char *)&fun_vars.var1, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"var2", (char *)&fun_vars.var2, SHOW_DOUBLE, SHOW_SCOPE_GLOBAL},
    {nullptr, nullptr, SHOW_UNDEF,
     SHOW_SCOPE_UNDEF}  // null terminator required
};

static SHOW_VAR show_array_fun[] = {
    {"array", (char *)show_status_fun, SHOW_ARRAY, SHOW_SCOPE_GLOBAL},
    {"var3", (char *)&fun_vars.var3, SHOW_CHAR, SHOW_SCOPE_GLOBAL},
    {"var4", (char *)&fun_vars.var4, SHOW_BOOL, SHOW_SCOPE_GLOBAL},
    {nullptr, nullptr, SHOW_UNDEF, SHOW_SCOPE_UNDEF}};

static SHOW_VAR func_status[] = {
    {"fun_func_fun", (char *)show_func_fun, SHOW_FUNC,
     SHOW_SCOPE_GLOBAL},
    {"fun_status_var5", (char *)&fun_vars.var5, SHOW_BOOL,
     SHOW_SCOPE_GLOBAL},
    {"fun_status_var6", (char *)&fun_vars.var6, SHOW_LONG,
     SHOW_SCOPE_GLOBAL},
    {"fun_status", (char *)show_array_fun, SHOW_ARRAY,
     SHOW_SCOPE_GLOBAL},
    {nullptr, nullptr, SHOW_UNDEF, SHOW_SCOPE_UNDEF}};

mysql_declare_plugin(fun){
    ...
    "Fun storage engine",
    PLUGIN_LICENSE_GPL,
    fun_init_func, /* Plugin Init */
    nullptr,           /* Plugin check uninstall */
    nullptr,           /* Plugin Deinit */
    0x0001 /* 0.1 */,
    func_status,              /* status variables */
    fun_system_variables, /* system variables */
